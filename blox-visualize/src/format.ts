...
export function createPriceFormat(priceScale: number): PriceFormat {
  const div = 10 ** priceScale;
  const dec = priceScale;
  const fmtPx = (p: number) =>
    (p / div).toLocaleString("en-US", { minimumFractionDigits: dec, maximumFractionDigits: dec });
  ...
  const fmtMoney = (tl: number) => fmtCurrency(tl / div);
