...
export class MarketChart {
  ...
  private posOpenedAt: number | null = null;
  ...
  setAvg(avg: number | null): void {
    ...
    this.posAvg = next;
    if (closed) this.posOpenedAt = null;
    else if (opened) this.posOpenedAt = Math.floor(Date.now() / 1000);
    ...
    if (opened || closed) this.repaint();
      ...
      for (const tf of this.tfList) {
        ...
        if (last === undefined || tf !== this.tf) continue;
  ...
  private vwapFor(key: number): number | null {
    if (this.posAvg === null || this.posOpenedAt === null) return null;
