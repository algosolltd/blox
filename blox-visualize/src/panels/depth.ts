...
export class Depth {
  ...
  private readonly maxRangePct: number;
  ...
  constructor(host: HTMLElement, opts: DepthOptions) {
    ...
    this.cv = q<HTMLCanvasElement>(host, ".depth-cv");
    this.tip = q(host, ".tip");
  ...
  private draw(): void {
    ...
    if (!bids.length && !asks.length) {
      ctx.fillStyle = "#5b6480"; ctx.font = "12px system-ui"; ctx.textAlign = "center";
      ctx.fillText("waiting for book…", w / 2, h / 2);
      return;
    }
    const cum = (lvls: BookLevel[]) => { let c = 0; return lvls.map(([p, qty]) => [p, (c += qty)] as BookLevel); };
    ...
    const X = (p: number) => padL + (p - pLo) / (pHi - pLo) * (w - padL - padR);
    const Y = (c: number) => padT + (1 - c / (maxCum * 1.06)) * (h - padT - padB);

    ctx.strokeStyle = "rgba(120,110,160,0.10)"; ctx.fillStyle = "#5b6480";
    ctx.font = "10px ui-monospace, Menlo, monospace"; ctx.lineWidth = 1; ctx.textAlign = "left";
    for (let i = 1; i <= 4; i++) {
    ...
    const area = (pts: BookLevel[], dir: number, rgb: string) => {
    ...
    if (this.hoverX != null) {
      ...
      const cumAt = (pts: BookLevel[], keep: (lp: number) => boolean) => {
        let c = 0;
      ...
      ctx.strokeStyle = "rgba(117,134,150,0.6)"; ctx.setLineDash([3, 3]);
      ctx.beginPath(); ctx.moveTo(x, padT); ctx.lineTo(x, h - padB); ctx.stroke();
      ctx.setLineDash([]);
      const dot = (c: number, rgb: string) => {
        if (c <= 0) return;
        ctx.beginPath(); ctx.arc(X(p), Y(c), 3.2, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(${rgb},1)`; ctx.fill();
        ctx.strokeStyle = "#0d1322"; ctx.lineWidth = 1.5; ctx.stroke();
      };
      dot(cb, "0,255,136"); dot(ca, "255,61,87");
