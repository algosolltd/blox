...
export class Orders {
  ...
  private readonly onCancel: (id: number) => void;
  private readonly onReduce: (id: number, qty: number) => void;
  private readonly ac = new AbortController();

  private open: OpenOrder[] = [];
  private closed: ClosedOrder[] = [];
  private reduceEdit: number | null = null;
  ...
  constructor(host: HTMLElement, opts: OrdersOptions) {
    ...
    this.openEl.addEventListener("focusout", (e) => {
      if ((e.target as HTMLElement).closest(".o-reduce-input")) this.revertReduceEdit();
    }, { signal });
  }
  ...
  private renderOpen(): void {
    ...
    if (this.reduceEdit != null) return;
    const rows = this.open;
    if (this.statusEl) this.statusEl.textContent = rows.length ? `${rows.length} working` : "";
    if (!rows.length) {
  ...
  private buildActions(id: number, remaining: number): HTMLElement {
  ...
  private initSplitter(bar: HTMLElement, closedSection: HTMLElement): void {
  ...
  private onClick(e: MouseEvent): void {
    ...
    if (cancel) this.onCancel(+cancel.dataset.cancel!);
    const rbtn = target.closest<HTMLElement>("[data-reduce]");
    if (!rbtn) return;
  ...
  private onKeydown(e: KeyboardEvent): void {
    const input = (e.target as HTMLElement).closest<HTMLInputElement>(".o-reduce-input");
    if (!input) return;
    if (e.key === "Enter") {
